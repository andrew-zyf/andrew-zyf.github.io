...
export const rowsFor = (compact: boolean) => compact ? ROWS * 2 : ROWS;
...
export const isDedication = (column: number, row: number, compact = false) => compact
  ? column >= 8 && column < 13 && row >= ROWS - 4 && row < ROWS + 4
  : column >= 16 && column < 24 && row >= ROWS / 2 - 3 && row < ROWS / 2 + 2;
...
let layouts: number[][] = [];
export function configureLayout(count: number) {
  ROWS = Math.max(10, Math.ceil((count + 40) / COLS / 2) * 2);
  layouts = [false, true].map(compact => {
  const slots: number[] = [];
  let account = 0;
...
export function overviewZoom(width: number, height: number): number {
  // Both arrangements reserve 40 cells for the dedication and fit the full snapshot.
  const compact = width < 600;
  const verticalMargin = compact ? 114 : 82;
