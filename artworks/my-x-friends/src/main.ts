...
async function main() {
  ...
  function select(index: number | null, cell?: [number, number]) {
    ...
    if (index !== null) {
      ...
      document.querySelector('#bubble-bio')!.textContent = (person.bio ? person.bio + (person.bioTranslated ? '\n\n由 X / Grok 翻译' : '') : '') || (person.bioStatus === 'empty' ? '这位朋友还没有填写个人简介。' : '暂时无法获取这位朋友的简介。');
  ...
  function updateSearch() {
    ...
    matches.forEach((index,position)=>{
      ...
      portrait.style.backgroundPosition=`${index%collection.atlasColumns/(collection.atlasColumns-1)*100}% ${Math.floor(index/collection.atlasColumns)/Math.max(1,collection.atlasRows-1)*100}%`;
